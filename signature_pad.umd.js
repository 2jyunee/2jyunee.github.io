(function (global, factory) {
  ...
  class Point {
    constructor(x, y, pressure, time) {
      ...
      this.pressure = pressure || lastPressure;
  ...
  function throttle(fn, wait = 5) {
    ...
    const later = () => {
      previous = Date.now();
      timeout = null;
      result = isLastPoint === false ? fn.apply(storedContext, storedArgs) : "";
  ...
  class SignaturePad extends SignatureEventTarget {
    ...
    _drawLastLine(event) {
      ...
      const pRadian2 = 225 * (Math.PI/180);
      const pX2 = (Math.cos(pRadian2) * (radius)) + circleX2;
      const pY2 = (Math.sin(pRadian2) * (radius)) + circleY2;
      let dots2 = [
